...
/*********************************************************/
int sm2_encrypt_data_test(const unsigned char *message, const int message_len,
                          const unsigned char *pub_key, unsigned char *c1,
                          unsigned char *c3, unsigned char *c2)
{
    int error_code;
    ...
    if (!(t = (unsigned char *)malloc((size_t)message_len)))
    {
        goto clean_up;
    }
    memset(t, 0, (size_t)message_len);
    if (!(ctx = BN_CTX_new()))
    {
        goto clean_up;
    }
    ...
    do
    {
        ...
        if (!(ECDH_KDF_X9_62(t, (size_t)message_len, x2_y2, sizeof(x2_y2), NULL, 0,
                             md)))
        {
    ...
    EVP_DigestUpdate(md_ctx, message, (size_t)message_len);
...
/*********************************************************/
int sm2_encrypt(const unsigned char *message, const int message_len,
                const unsigned char *pub_key, unsigned char *c1,
                unsigned char *c3, unsigned char *c2)
{
    int error_code;
...
/*********************************************************/
int sm2_decrypt(const unsigned char *c1, const unsigned char *c3,
                const unsigned char *c2, const int c2_len,
                const unsigned char *pri_key, unsigned char *plaintext)
{
    int error_code;
    ...
    memset(t, 0, (size_t)message_len);
    if (!(ECDH_KDF_X9_62(t, (size_t)message_len, x2_y2, sizeof(x2_y2), NULL, 0, md)))
    {
        error_code = COMPUTE_SM2_KDF_FAIL;
        goto clean_up;
    }
    ...
    if (!(M = (unsigned char *)malloc((size_t)message_len)))
    {
        goto clean_up;
    }
    for (i = 0; i < message_len; i++)
    ...
    EVP_DigestInit_ex(md_ctx, md, NULL);
    EVP_DigestUpdate(md_ctx, x2, sizeof(x2));
    EVP_DigestUpdate(md_ctx, M, (size_t)message_len);
    EVP_DigestUpdate(md_ctx, y2, sizeof(y2));
    EVP_DigestFinal_ex(md_ctx, digest, NULL);

    if (memcmp(digest, c3, sizeof(digest)) != 0)
    {
        error_code = INVALID_SM2_CIPHERTEXT;
        goto clean_up;
    }
    memcpy(plaintext, M, (size_t)message_len);
    error_code = 0;
